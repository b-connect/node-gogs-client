...
function API (apiUrl, requester) {
  ...
  /**
   * List all organisations for a user
   * @param user {object} the user to query. Requires username
   * @param authUser {object} the user authenticating this request. Requires token or username and password
   * @return {Promise<object>} list of all organisations
   */
  _this.listOrganisations = function (user, authUser) {
    return request('user/orgs', authUser).then(stat.checkStandardResponse);
  };
   ...
   * @return {Promise<object>} list of all organisations
   */
  _this.createOrganisation = function (user, org, authUser) {
    return request('orgs/' + user.username , authUser, org).then(stat.checkCreatedResponse);
  };

  /**
   * Get a organisation
   * @param org {object} the organisation to query. Requires id
   * @param authUser {object} the user authenticating this request. Requires token or username and password
   * @return {Promise<object>} the organisation object
   */
  _this.getOrganisation = function (org, authUser) {
    return request('orgs/' + org.username , authUser).then(stat.checkStandardResponse);
  };

  /**
   * Update a organisation
   * @param org {object} the organisation to query. Requires full_name, description, website, location
   * @param authUser {object} the user authenticating this request. Requires token or username and password
   * @return {Promise<object>} the updated organisation object
   */
  _this.editOrganisation = function (org, authUser) {
    return request('orgs/' + org.username , authUser, org, 'PATCH').then(stat.checkStandardResponse);
  };

  /**
   * Migrate a repository
   * @param uid {object} the organisation to query. Requires username
   * @param repo {object} the organisation to query. Requires username
   * @param authUser {object} the user authenticating this request. Requires token or username and password
   * @return {Promise<object>} the updated organisation object
   */
  _this.migrateRepo = function ( repo, authUser) {
    return request('repos/migrate' , authUser, repo).then(stat.checkStandardResponse);
  };
